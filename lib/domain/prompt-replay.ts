...
const fieldMap = fixtureJson.fieldMap as Record<string, FinalOutputField>;
...
export function replayPromptResult(input: ExecuteInitialMatchingInput, fixture = promptFixture): ExecuteInitialMatchingResult {
  ...
  if (pool.length !== fixture.sourceLines.length || pool.some(s => !fixture.sourceLines.some(f => f.id === s.id))) throw new Error('查货候选范围已变化，需要重新运行 Prompt');
  ...
  for (const line of input.entrustmentLines) {
    const baseline = fixture.orderRows.find(r => r.id === line.id);
    ...
    if (line.matchRelationIds.length) {
      const patch = fixture.rowPatches.find(p => p.order_row_id === line.id);
      for (const decision of patch?.field_decisions ?? []) expected[fieldMap[decision.field]] = decision.result_value || null;
    }
  ...
  const entrustmentLines = input.entrustmentLines.map(line => {
    ...
    const verdict = fixture.relations.find(r => r.order_row_id === line.id);
    ...
    const target = fixture.verificationTargets.find(t => t.order_row_id === line.id);
    const patch = fixture.rowPatches.find(p => p.order_row_id === line.id);
    ...
    for (const logicalId of new Set(selected.map(s => s.logicalInspectionOrderId))) {
      const relationId = `MR-${fixture.auditId}-${line.id}-${logicalId}-v${input.draft.version+1}`;
      relationIds.push(relationId);
      relations.push({id:relationId,draftId:input.draft.id,entrustmentLineId:line.id,logicalInspectionOrderId:logicalId,inspectionMergedProductId:null,inspectionSourceLineIds:selected.filter(s=>s.logicalInspectionOrderId===logicalId).map(s=>s.id),establishedBy:'AI',active:true,createdAt:input.now,invalidatedAt:null,invalidationReason:null,evidenceSummary:`P3 ${fixture.auditId}：${verdict.evidence_keys.join('、')}`,modelCoverage:verdict.coverage,modelEvidenceKeys:verdict.evidence_keys});
    }
    ...
    for (const decision of patch.field_decisions) {
      const field = fieldMap[decision.field];
      const targetField = (target.fields as Record<string, {current_value:string;evidence:Array<{evidence_key:string;source:{file_id:string;page?:number;sheet?:string;row?:number|null;raw_text:string}}>}>)[decision.field];
      ...
      const sourceLocation: SourceLocation = raw ? {fileId:raw.file_id,page:raw.page??null,sheet:raw.sheet??null,position:raw.row?`第 ${raw.row} 行`:raw.raw_text} : selected[0].sourceLocation;
...
export function initialPromptEvidence(): FieldEvidence[] {
  return promptFixture.orderRows.flatMap(row => row.facts.evidence.map((fact,index) => ({
    id:`FE-${promptFixture.auditId}-P1-${row.id}-${index}`,draftId:promptFixture.draftId,entrustmentLineId:row.id,field:fieldMap[fact.field],currentValue:fact.value||null,originalValue:fact.value||null,sourceMaterialType:'委托书' as const,sourceFileId:fact.source.file_id,
    sourceLocation:{fileId:fact.source.file_id,page:fact.source.page,sheet:fact.source.sheet,row:fact.source.row,column:fact.source.column,rawText:fact.source.raw_text,position:`第 ${fact.source.row} 行 ${fact.source.column}`},sourceInspectionLineId:null,isAiUpdated:false,isManuallyEdited:false,hadConflict:false,candidateValues:[],modelAuditId:promptFixture.auditId,
  })));
}
