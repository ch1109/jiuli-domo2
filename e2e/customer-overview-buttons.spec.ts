...
test('客户业务概览四级交互定稿规范端到端测试', async ({ page }) => {
  ...
  await expect(puyiCard.getByRole('button', { name: '处理商品对应' })).toBeVisible();
  await expect(puyiCard.getByRole('button', { name: '查看商品对应 →' })).toBeVisible();
  ...
  await expect(bmhCard.getByRole('button', { name: '选择对应商品' })).toBeVisible();
  await expect(bmhCard.getByRole('button', { name: '查看商品对应 →' })).toBeVisible();
  ...
  await expect(acsyCard.getByRole('button', { name: '查看商品对应 →' })).toBeVisible();
  ...
  await expect(yingbaoCard.getByRole('button', { name: '查看商品对应 →' })).toBeVisible();
  ...
  await expect(oultCard.getByRole('button', { name: '查看商品对应 →' })).toBeVisible();
  ...
  await expect(yingkaCard.getByRole('button', { name: '查看全部商品对应 →' })).toBeVisible();
  await expect(yingkaCard.getByRole('button', { name: '查看全部 3 票 ›' })).toBeVisible();

  // 8. 交互测试：点击浦壹卡片的“查看商品对应 →”，成功进入商品对应面板与规范副标题
  await puyiCard.getByRole('button', { name: '查看商品对应 →' }).click();
